...
Cloudinary driver no longer stores a truncated or gapped asset when a chunked write's last chunk finishes first or an earlier chunk fails, and overwrites, renames and deletes now invalidate the CDN so `read` stops serving stale or deleted content.
