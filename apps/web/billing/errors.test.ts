...
describe('LimitExceededError', () => {
	test('Constructs message', () => {
		// The message names the entitlement the limit was exceeded for
	...
	test('Carries the code, the status and the category', () => {
		// The transport layer reads the code, the HTTP status and the extensions off the error
...
describe('ResourceRestrictedError', () => {
	test('Constructs message', () => {
		// The message names the feature the plan does not grant
		expect(resourceRestrictedMessage({ category: 'sso' })).toMatchInlineSnapshot('"Resource "sso" is restricted."');
	});

	test('Carries the code, the status and the category', () => {
		// The same shape as the limit error, with its own code
