...
describe('PlanCatalog', () => {
	// The catalog under test is built the way the app builds it: validated definitions in, lookups out
	...
	test('Resolves the provider price id of a plan and period, and complains when it is missing', () => {
		// A price id resolves for the plan and period that carry it, per provider
		expect(catalog.priceIdOf('pro', 'yearly', 'stripe')).toBe('price_pro_y');
		expect(catalog.priceIdOf('pro', 'monthly', 'polar')).toBe('prod_pro_m');

		// A missing price, a missing provider id, and a free plan without prices are three distinct complaints
		expect(() => catalog.priceIdOf('business', 'yearly', 'stripe')).toThrow('Plan "business" has no yearly price');
		expect(() => catalog.priceIdOf('business', 'yearly', 'stripe')).toThrow(InvalidPayloadError);
		...
		expect(() => catalog.priceIdOf('business', 'monthly', 'polar')).toThrow(InvalidConfigError);
	...
	test('Maps a provider price id back to its plan and period', () => {
		// The webhook's direction: a price id from the provider names the plan and period it belongs to
		...
		// An id of another provider, or one nobody uses, maps to nothing
	...
	test('Reads entitlements: limits, no limit, switches, and absence', () => {
		// A number is a limit, null is no limit, a boolean a switch; a key the plan omits is not granted
		...
		// The keys of every plan, in definition order
		expect(catalog.entitlementKeys()).toStrictEqual(['seats', 'projects', 'sso']);
	});

	test('Exposes the plans as plain data', () => {
		// Serialization loses nothing: what pages render travels as data
