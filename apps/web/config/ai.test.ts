...
describe('aiConfig', () => {
	test('Registers nothing without a key', () => {
		// 1. A fresh clone runs without AI: no provider, and no alias that would point at one
		expect(aiConfig(env())).toStrictEqual({ providers: {}, models: {} });
	});

	test('Registers a provider per key that is set', () => {
		// 1. Every key brings its provider under the name the model ids use
	...
	test('Points each alias at the first provider serving it', () => {
		// 1. The gateway goes first when it is set
		...
		// 2. Anthropic serves chat but no embeddings, so those go to the next provider that has them
		expect(aiConfig(env({ AI_ANTHROPIC_API_KEY: 'sk-anthropic', AI_GOOGLE_API_KEY: 'google' })).models).toStrictEqual({
			chat: 'anthropic:claude-sonnet-5',
			embeddings: 'google:gemini-embedding-001',
		});

		// 3. With Anthropic alone, embeddings stay unset rather than pointing at a provider without a key
	...
	test('Resolves every default alias through the real providers', () => {
		// 1. Each provider set, each preference order tried: the providers only build model objects here, so no
		//    request leaves the process, yet a wrong model kind or a provider without it would throw
