...
describe('smsConfig', () => {
	test('Defaults to the console driver outside production', () => {
		// Without SMS_DRIVER in test, the location rides the console driver, so a fresh clone reads its one-time codes
		// in the terminal — and the sender comes from SMS_FROM
	...
	test('Refuses to boot in production without SMS_DRIVER', () => {
		// The silent console default must not reach production: boot fails loudly instead of logging every code
		expect(() => smsConfig(env({ NODE_ENV: 'production' }))).toThrowError(/SMS_DRIVER is required in production/);
		expect(() => smsConfig(env({ NODE_ENV: 'production' }))).toThrowError(InvalidConfigError);
	});
