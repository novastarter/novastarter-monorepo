 ...
 * @throws InvalidConfigError in production without `SMS_DRIVER`: booting would deliver nothing and log every one-time code
 * instead.
 */
export const smsConfig = (env: AppEnv): SmsConfig => {
	// Outside production the console driver is the default, so a fresh clone reads its one-time codes in the terminal;
	// production must name a driver explicitly — a silent console default would log every code instead of delivering it
