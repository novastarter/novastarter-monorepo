...
describe('migrateDatabase on PGlite', { timeout: 30_000 }, () => {
	...
	afterAll(async () => {
		...
		// The boot flags, the parsed env and the database manager reset, so a later suite boots from nothing
		_state.booted = false;
		_state.handlers = false;
		readEnv.reset();
		useDatabase.reset();

		// The environment stubs of the suite, so the next one reads the real shell again
		vi.unstubAllEnvs();
	});

	test('Applies the committed folder and the schema matches it', async () => {
		// Twice, so a second start finds nothing pending
		await migrateDatabase();
		await migrateDatabase();

		// A row through the typed schema proves the table the migration created is the one `db/schema.ts` describes
		const db = useDb();
