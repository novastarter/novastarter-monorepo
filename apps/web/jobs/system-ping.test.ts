...
describe('system.ping', () => {
	test('Is registered with the queue when the module loads', () => {
		// Importing the module is what makes `enqueue('system.ping', …)` known; nothing else registers it
	...
	test('Defaults the message and validates the stamp', () => {
		// An empty payload becomes the default ping
		expect(systemPing.parse({})).toStrictEqual({ message: 'ping' });

		// The stamp must be ISO 8601 — the schema names it in the error
	...
	test('Schedules itself every five minutes in development only', () => {
		...
		// The schedule is data the worker's `startSchedules()` reads; the switch is the environment's
...
describe('createSystemPingHandler', () => {
	test('Logs the message, the id, the wait since the enqueue and the attempt', async () => {
		...
		// One ping through the handler writes exactly one line, with the message, the id and the attempt
		...
		// The wait is measured from the enqueue, so it is at least the 250 ms the context claims
	...
	test('Measures from the payload’s own stamp when it carries one', async () => {
		...
		// A ping stamped five seconds ago measures its wait from the stamp, not from the context's enqueue time
	...
	test('Writes to the app’s logger when none is given', async () => {
		// The app logger is the mocked `useLogger()`, so its return value is where the handler's line lands
		const logger = { info: vi.fn() };

		vi.mocked(useLogger).mockReturnValue(logger as never);

		// No logger passed: the handler asks `useLogger()` and writes the ping there
