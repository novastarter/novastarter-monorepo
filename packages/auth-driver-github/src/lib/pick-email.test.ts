...
describe('pickEmail', () => {
	...
	test('Picks nothing when the primary address is unverified, even with another verified one', () => {
		// A verified secondary is not the person's chosen address; an unverified primary must not link accounts
		expect(
			pickEmail([
	...
	test('Picks nothing from a body that is not a list or from entries of the wrong shape', () => {
		// The list comes from the network, so truthy strings must not pass for booleans and junk entries are skipped
