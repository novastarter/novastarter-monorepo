...
describe('request', () => {
	...
	test('Throws a provider failure for a network error and for a request past its deadline', async () => {
		...
		expect(error).toMatchObject({
			message: 'The google sign-in failed: the request to https://x.test failed: ECONNRESET',
			cause: socket,
		});
...
describe('toJwksFetch', () => {
	...
	test('Gives a fake answer the JSON reader jose needs', async () => {
		// A narrow fake answers status and text only; `createRemoteJWKSet` still reads the key set out of it
