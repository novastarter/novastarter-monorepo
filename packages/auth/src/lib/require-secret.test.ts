...
describe('requireSecret', () => {
	...
	test('Refuses a missing, empty or short secret, naming the setting', () => {
		...
		// There is no safe default for a secret, and one under the minimum is not random data
		...
		expect(() => requireSecret(undefined, 'oauth.secret')).toThrow(expect.objectContaining({ code: 'INVALID_CONFIG' }));
	});
});
...
describe('requireSecrets', () => {
	...
	test('Refuses a missing or empty list, and a short secret anywhere in it', () => {
		...
		// An old secret still opens stored values, so it is held to the same bar as the current one
