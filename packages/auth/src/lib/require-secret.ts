 ...
 * @returns The secret.
 * @throws InvalidConfigError when it is missing or shorter than {@link MIN_SECRET_LENGTH}: there is no safe default for
 * a secret, and a short one can be brute-forced offline from anything it signed or encrypted.
 * @internal
 */
export const requireSecret = (secret: string | undefined, setting: string): string => {
	// Refused rather than defaulted, and refused when short: both mistakes would otherwise go unnoticed
	if (!secret || secret.length < MIN_SECRET_LENGTH) {
		throw new InvalidConfigError({
			reason: `The "${setting}" auth setting must be at least ${MIN_SECRET_LENGTH} characters of random data`,
		});
	}
 ...
 * @throws InvalidConfigError when there is none, or any of them is shorter than {@link MIN_SECRET_LENGTH}: an old
 * secret kept for rotation still opens stored values, so it has to be as strong as the current one.
 * @internal
 */
export const requireSecrets = (secrets: string | readonly string[] | undefined, setting: string): string[] => {
	// One string is the common case, a list with no rotation in progress; an empty list is a missing secret
