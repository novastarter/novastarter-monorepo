...
describe('hotp', () => {
	test.each(RFC_VECTORS)('Gives the RFC 6238 code at %i seconds: %s', (seconds, code) => {
		// TOTP is HOTP of the time step; the vectors pin the counter encoding, the HMAC and the truncation
		expect(hotp(RFC_SECRET, totpStep(seconds * 1000))).toBe(code);
	});

	test('Gives the RFC 4226 HOTP vectors for the first counters', () => {
		// RFC 4226 appendix D, same secret: the zero padding of `005924`-like codes is covered by these too
...
describe('otpauthUri', () => {
	test('Puts the issuer in the label and the parameters, with the algorithm, digits and period', () => {
		...
		// The issuer is repeated for the apps that ignore the label
