...
const wrongCodeFor = (secret: string): string => {
	// Counted up from zero until it misses every step of the window, so the test never hits a right code by chance
...
describe('verifyTotp', () => {
	...
	test('Refuses a right code whose step did not advance', async () => {
		...
		// A used step is refused, so a code seen over a shoulder is useless a moment later
		await expect(
	...
	test('Refuses to run without a usable encryption key', async () => {
		...
		// The key gone from the settings is a configuration error, not a wrong code
