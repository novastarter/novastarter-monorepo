...
describe('hashPassword', () => {
	...
	test('Uses the default cost when none is given', async () => {
		// The default cost runs within the `maxmem` the module computes; otherwise Node would refuse it
		...
		// The default cost takes about half a second alone, but far longer when `pnpm test` runs every package at once
	}, 30_000);

	test('Refuses an empty password and one longer than the maximum', async () => {
		// Both are rejected before any hashing, as a payload error the caller can show
...
describe('normalizePassword', () => {
	test('Folds a decomposed character into its composed form', () => {
		// "é" typed as `e` + combining acute and as one code point is the same password
		expect(normalizePassword('café')).toBe('café');
	});

	test('Refuses a value that is not a string', () => {
		// A form field parsed as a number or `null` must not reach scrypt
