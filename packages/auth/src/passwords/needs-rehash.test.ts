...
describe('needsRehash', () => {
	test('Is false for a hash made with the current cost and true for any other cost', async () => {
		...
		// A higher and a lower value of each parameter both count, since the configured cost is the one wanted
	...
	test('Compares with the default cost when none is given', () => {
		// Parsing needs no scrypt run, so hand-written hashes are enough here
	...
	test('Throws on a hash that is not a scrypt PHC string', () => {
		// A broken record is not silently treated as needing a rehash
