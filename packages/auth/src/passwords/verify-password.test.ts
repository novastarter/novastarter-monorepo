...
describe('verifyPassword', () => {
	...
	test('Verifies a hash made with another cost than the default', async () => {
		// The cost travels in the hash, so raising the default does not break old hashes
	...
	test('Throws on a broken hash rather than answering false', async () => {
		// A corrupt record is an error to look into, not a wrong password to show the user
	...
	test('Throws on a hash whose cost is out of bounds before running scrypt', async () => {
		// A tampered `ln` could otherwise make one verification allocate gigabytes
