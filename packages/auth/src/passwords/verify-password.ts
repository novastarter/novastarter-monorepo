...
export const verifyPassword = async (password: string, hash: string): Promise<boolean> => {
	// The stored hash is taken apart first, so a broken record fails loudly rather than as a wrong password
	const parsed = parseHash(hash);

	// The same normalisation and cost as when the hash was made
	const key = await derive(normalizePassword(password), parsed.salt, parsed.params);

	// Compared in constant time; lengths differ only for a hash made with another key length, which cannot match
