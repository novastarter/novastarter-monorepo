...
describe('encodeBase32', () => {
	test.each(VECTORS)('Encodes %j as the RFC 4648 vector %j without padding', (input, expected) => {
		// The published vectors pin the alphabet, the bit order and the handling of the last partial character
...
describe('decodeBase32', () => {
	test.each(VECTORS)('Decodes back to %j from %j', (expected, input) => {
		// The trailing zero bits of the last character are padding, not an extra byte
	...
	test('Refuses a character outside the alphabet', () => {
		// `1`, `8`, `0` and `9` are not base32; a silent skip would decode a different secret
		expect(() => decodeBase32('MZXW1')).toThrow('@novastarter/auth: invalid base32 character "1"');
	});

	test('Roundtrips random bytes of every length', () => {
		// Every remainder of the 5-byte block is covered, so no length loses or gains a byte
