...
test('Lists the JavaScript extensions a config file may have', () => {
	// `@novastarter/env` picks the JS reader by membership in this list, so the members are the contract; the exact literal also pins the documented `@defaultValue`
	expect(JAVASCRIPT_FILE_EXTS).toEqual(['js', 'mjs', 'cjs']);
});

test('Defaults the chunk size to 8 MiB', () => {
	// Storage drivers size their resumable-upload parts by it; a change here changes every upload
