...
describe('#label', () => {
	...
	test('Leaves the logger as it is without a label', () => {
		...
		new DatabaseDriverPglite({ connection: MEMORY_DATA_DIR, logger: logger as never });
...
describe('#ping', () => {
	...
	test('Wraps what the query raised in DatabaseUnavailableError, naming the location', async () => {
		...
		expect(thrown).toMatchObject({
			...
			status: 503,
			message: 'Database "main" is unavailable: PGlite is closed',
			extensions: { database: 'main', reason: 'PGlite is closed' },
			cause: error,
		});
	});
	...
	test('Reads without a location for a driver built by hand', async () => {
		...
		sample.db.execute.mockRejectedValue(new Error('PGlite is closed'));

		await expect(driver.ping()).rejects.toThrow('The database is unavailable: PGlite is closed');
	});
});
