...
describe('useEmitter', () => {
	test('Builds the emitter on first use and hands the same one out afterwards', () => {
		// Built once, handed out forever: the emitter keeps its listeners between calls
	...
	test('Builds a fresh emitter after reset()', () => {
		// After a reset the next call builds a fresh emitter — new listeners, no residue of the old
