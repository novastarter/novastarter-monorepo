...
test('Keeps CONFIG_PATH relative, so the working directory at lookup time decides', () => {
	// The default is the conventional dotenv name, resolved lazily by getConfigPath; resolving it here at import
...
test('Is typed as the parsed configuration', () => {
	// Defaults are values of the same map the environment is parsed into, so the two types must line up exactly
