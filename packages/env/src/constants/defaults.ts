...
/**
 * Fallback value for every configuration option that has one.
 *
 * Applied before the process environment and the config file, so anything set there wins. Values are stored in
 * their raw form and only cast when the variable has an entry in the type map, which is why lists appear as
 * comma-separated strings here. `Infinity` marks a limit that is off by default.
 *
 * @defaultValue See the object body; `CONFIG_PATH` resolves to `.env` in the working directory.
 */
export const DEFAULTS: Env = {
	...
	EXTENSIONS_STORAGE_MAX_CONCURRENCY: 20,
	...
	ASSETS_TRANSFORM_IMAGE_MAX_OUTPUT_DIMENSION: 6000,
