...
test('Lists every cast flag the package reads', () => {
	// 1. The list is the contract: a value prefixed with one of these is coerced, anything else before a colon is
	//    plain data
	expect(ENV_TYPES).toStrictEqual(['string', 'number', 'regex', 'array', 'json', 'boolean']);
});

test('Is a readonly tuple of the literal flags', () => {
	// 1. `as const` keeps the element type to the six literals, which is what `getCastFlag` promises its callers; a
