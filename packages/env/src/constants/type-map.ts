...
/**
 * Environment variables that are expected to hold a certain type.
 *
 * Sets the default casting of these values when no explicit cast prefix is given. Keys are regular-expression
 * sources, so `STORAGE_.+_SECRET` covers every storage location. The main reason for listing a `string` here is to
 * stop the type guesser from turning a numeric-looking password or port into a number.
 */
...
/**
 * {@link TYPE_MAP} compiled to anchored regular expressions, in declaration order.
 *
 * Compiled once at module load, so the per-variable lookup at startup only runs `test()`.
 */
