...
beforeEach(() => {
	// Every reader returns a distinctive object, so each test sees at a glance which one was picked
...
afterEach(() => {
	// resetAllMocks also drops the programmed returns above, so the next test reprograms them from scratch
	vi.resetAllMocks();
});

test('Returns null if file path does not exist', () => {
	// A missing config file is not an error — it means the application runs on the process environment alone
...
test('Reads JS file if extension is js', () => {
	// The extension alone routes the file, no content sniffing
...
test('Reads JSON file if extension is json', () => {
	// JSON is routed to the require-based reader
...
test('Reads yaml file if extension is yaml or yml', () => {
	// Both spellings of the YAML extension route to the same reader
...
test('Reads from dotenv file if extension is unknown or missing', () => {
	// The plain `.env` file has no extension at all; dotenv syntax is the fallback for anything unrecognized
