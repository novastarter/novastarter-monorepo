...
test('Returns false if key is less than or equal to 5 in length', () => {
	// `_FILE` alone is five characters and must stay a plain variable name; anything at or below the suffix
...
test('Returns false if key does not end with _FILE', () => {
	// A plain variable is not a file reference, even when it is long enough to hold the suffix
	expect(isFileKey('TEST_123')).toBe(false);
});

test('Returns true if key is longer than 5 characters and ends in _FILE', () => {
	// A name with something before the suffix is the `*_FILE` form of a variable
