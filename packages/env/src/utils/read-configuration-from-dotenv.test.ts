...
afterEach(() => {
	// Mocked fs and dotenv must not leak their programmed returns into the next test
	vi.clearAllMocks();
});

test('Reads file contents of path synchronously', () => {
	// The file is read with a blocking call, because configuration is needed before anything async can run
...
test('Parses file contents with dotenv', () => {
	// The raw text goes through dotenv's parser rather than a hand-rolled one, so quoting rules match the format
...
test('Returns parsed dotenv config', () => {
	// Whatever dotenv parsed is the configuration, passed on by reference
