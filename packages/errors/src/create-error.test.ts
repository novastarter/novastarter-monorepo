...
beforeEach(() => {
	// The code is lower-case on purpose, to prove the factory upper-cases it
	sample = {
		code: 'test_error',
		status: 400,
...
test('Returns enhanced error object', () => {
	// The shared shape every error made by the factory carries: name for the guard, code for matching, status
...
test('Allows passing message as callback function', () => {
	// A function message is resolved per instance, so the text can depend on the extensions
	const TestError = createError(sample.code, () => sample.message, sample.status);
	const error = new TestError();
	expect(error.message).toBe(sample.message);
});
...
test('Passes extensions to custom message function', () => {
	// The extensions are handed to the message function, so the text can name what failed
...
test('Makes extensions available', () => {
	// The same object the caller passed is kept on the instance, by reference, for consumers reading after catch
	const messageGenerator = vi.fn().mockReturnValue(sample.message);
	const extensions = { [sample.extensionKey]: sample.extensionValue };
	const TestError = createError<{ [key: string]: unknown }>(sample.code, messageGenerator, sample.status);
...
test('Overrides toString', () => {
	// Logs read `name [CODE]: message`, so the code is findable without extra formatting
	const TestError = createError(sample.code, sample.message, sample.status);
