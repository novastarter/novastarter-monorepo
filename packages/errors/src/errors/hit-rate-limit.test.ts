...
beforeAll(() => {
	// A fixed clock, so the duration in the message is stable enough to snapshot
	vi.useFakeTimers();
	vi.setSystemTime('2023-05-31T14:45:00Z');
});

afterAll(() => {
	// The fake clock must not leak into the other test files of the process
	vi.useRealTimers();
});

test('Constructs message', () => {
	// Thirty seconds after the fixed now, reported in a human-readable duration
	expect(
		hitRateLimitMessage({
			limit: 100,
...
test('Clamps a reset in the past to "retry now"', () => {
	// A stale reset is answered with a zero wait rather than a negative duration the client cannot act on
	expect(
		hitRateLimitMessage({
			limit: 100,
...
test('Treats an invalid reset Date as "retry now" instead of throwing', () => {
	// `ms(NaN)` throws, and the error constructor must not blow up while reporting another error
	expect(
		hitRateLimitMessage({
			limit: 100,
