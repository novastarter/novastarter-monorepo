...
describe('FeatureFlagsDriverStatic', () => {
	...
	test('Refuses an invalid definition and a duplicate key', () => {
		// A bad key or an out-of-range percentage fails at construction, i.e. on the location's first use
		...
		// Two definitions of one key would make the winner depend on their order
		expect(
			() =>
			...
			`[NovastarterError: Invalid config. Feature flag "a" is defined twice; keep one definition per key.]`,
		);
	});
});
