...
describe('parseCallMethod placeholders', () => {
	test('Fills {name} from the parameters, encoded, and leaves the rest for the query or body', () => {
		// 1. Octokit's shape: the path takes its parameters, the others stay
		expect(
		...
		// 2. A value cannot reshape the path, and a number is written as digits
	...
	test('Leaves a placeholder without a parameter for the driver, and refuses an object for one', () => {
		// 1. `{bucket}` is the driver's to fill; the caller's parameters are not touched
		...
		// 2. A list or an object has no single place in a path
	...
	test('Refuses a value that would move the request to another path, and ignores inherited names', () => {
		// 1. `..` or `.` would climb the path once the URL collapses it; an empty one would hit the collection
		for (const id of ['..', '.', '']) {
			expect(() => parseCallMethod('DELETE /files/{id}', { id })).toThrow('cannot be empty, "." or ".."');
		}

		// 2. A name only the prototype has is not a parameter: the placeholder stays for the driver
	...
	test('Fills the same placeholder twice with one parameter, and takes 0 and false as values', () => {
		// 1. Repeated: both places filled, the parameter still sent once — not at all
...
describe('toHeaderRecord', () => {
	test('Lower-cases names from a Headers or a record, joining repeats and lists', () => {
		// 1. A `Headers`, a header given twice
		...
		// 2. A record with a list, a number and a hole; nothing at all
