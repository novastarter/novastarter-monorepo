...
describe('resolveCallUrl', () => {
	...
	test('Keeps a query of the base after the target’s own, and reads allowed hosts in any case', () => {
		// An API version in the base survives, the target's own parameter wins
		...
		// A host listed in upper case still matches the lower-cased URL host
	...
	test('Allows plain HTTP only on the base’s own host', () => {
		// A stand-in base in tests may be HTTP; the provider's real hosts are never reached without TLS
	...
	test('Refuses a path with a placeholder nobody filled', () => {
		// It would reach the provider as `%7Bowner%7D`
	...
	test('Refuses a full URL on another host, or plain HTTP under an HTTPS root', () => {
		// The credentials would go to someone else
		expect(() => resolveCallUrl('https://api.stripe.com', 'https://evil.example/v1', ['files.stripe.com'])).toThrow(
			'@novastarter/http: the call URL is not on a host of this provider: evil.example',
		);
...
describe('toQueryString', () => {
	...
	test('Sends a date as its ISO string, without JSON quotes', () => {
		// A date is an object, but the API wants the bare timestamp
...
describe('httpCall', () => {
	...
	test('Sends the parameters of a GET in the query and parses a JSON answer', async () => {
		...
		// The query is added to what the URL already carries
		await expect(
			httpCall({
	...
	test('Sends JSON, a form, or multipart when a file is among the parameters', async () => {
		...
		// JSON by default, with its content type; an empty answer is nothing
		await expect(
			httpCall({
		...
		// A form where the API wants one
		fetchMock.mockClear();
		...
		// A file: multipart, a JSON type set by the driver dropped so `fetch` sets the boundary
		fetchMock.mockClear();
	...
	test('Answers a text body as text, whatever the status', async () => {
		...
		// The status is the driver's to judge
		await expect(
	...
	test('Follows a redirect on the same origin with the credentials, and one to another origin without them', async () => {
		...
		// Three hops: the key is sent twice on the provider's host, and never to the CDN
		await expect(
			httpCall({
	...
	test('Turns a 303 into a GET without a body, and stops after too many redirects', async () => {
		...
		// The POST's body is not sent again
		await httpCall({
			url: new URL('https://api.example/x'),
			verb: 'POST',
			params: { a: 1 },
		...
		// A loop ends in an error rather than forever
		fetchMock.mockReset();
	...
	test('Keeps a HEAD as a HEAD across a 303', async () => {
		...
		// The redirected request must not become a GET, or the server may send a full body the caller never asked for
		await httpCall({
			url: new URL('https://api.example/x'),
			verb: 'HEAD',
			timeout: 1000,
		...
		// The second hop repeats the HEAD, the way fetch and browsers follow a 303
	...
	test('Repeats a list of files, one part each, and leaves null out', async () => {
		...
		// A list of files: one part each; `null` left out
		await httpCall({
			url: new URL('https://api.example/x'),
			verb: 'POST',
	...
	test('Keeps the deadline while the body is read', async () => {
		// Headers arrive at once, the body never ends: the deadline still fires
	...
	test('Lets the caller’s content type pick the body: a form, multipart, a vendor JSON type', async () => {
		...
		const post = async (type: string, params: Record<string, unknown>) => {
			// A fresh call each time, read back from the stub
			fetchMock.mockClear();
		...
		// A form, whatever the driver's default
		expect((await post('application/x-www-form-urlencoded; charset=utf-8', { token: 't', hint: 'access' })).body).toBe(
			'token=t&hint=access',
		);

		// Multipart without a file, the type left to `fetch` for its boundary
		...
		// A vendor JSON type is JSON, and stays the type sent
	...
	test('Refuses to follow a redirect to another origin with the body, or down to plain HTTP', async () => {
		// A 307 keeps the method and the body: to another host that body would go to someone else
		...
		// Down to plain HTTP on another host
	...
	test('Keeps a scalar under a text type as text, and parses JSON under any type', async () => {
		// A text answer that looks like a number stays text
		...
		// A vendor JSON type is parsed, and so is an object labelled as text
