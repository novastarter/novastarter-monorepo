...
const readFile = async (
	...
	...file,
	// A `Blob` has no own properties to compare, so its bytes and type are taken out
...
describe('toMailgunTags', () => {
	test('Cuts every label to the length limit, drops an empty one and caps the count, the category first', () => {
		// Mailgun refuses a message past its tag limits, so the labels are adapted instead: the category leads, an
	...
	test('Brings a tag into Mailgun character set instead of refusing the message over it', () => {
		// Mailgun takes ASCII letters, digits, `_` and `-` only, so a non-ASCII label becomes underscores rather than
		// failing the whole send; the sanitised label is then cut and capped like any other
...
describe('toMailgunHeader', () => {
	...
	test('Refuses a name that is no token, with a colon or whitespace in it', () => {
		// A name holding a colon would make Mailgun's raw header name two headers; whitespace or a folded name is no
		// single header either — the name has to be one RFC 7230 token
		expect(() => toMailgunHeader('X-Injected: Bcc', 'x')).toThrow(InvalidPayloadError);
	...
	test('Refuses a value with CR or LF in it, which would forge a header line', () => {
		// A line break in the value reaches Mailgun's raw header verbatim, so a value of more than one line is refused;
		// the same goes for other control characters, a tab excepted
...
describe('toMailgunFile', () => {
	test('Keeps inline content and takes the content id as the filename', async () => {
		// Mailgun matches `cid:` references by filename, so the content id stands in for it
		expect(
	...
	test('Carries the content type into the multipart part the SDK builds', async () => {
		// The file the way the mapper builds an inline image named by a bare content id, so the filename gives Mailgun
		// no type to guess
		...
		// Append it the way mailgun.js does with Node's own FormData — a `Blob` goes in unchanged with the filename —
	...
	test('Reads a path', async () => {
		// This very file is the attachment; a Buffer proves the path was read
...
describe('toMailgunMessage', () => {
	test('Maps the message into Mailgun form fields, inline files apart', async () => {
		...
		// The files are typed `Blob`s, read back into bytes to compare the whole payload by value
	...
	test('Requires a sender', async () => {
		await expect(toMailgunMessage({ to: 'a@example.com', subject: 'S' })).rejects.toThrow(InvalidPayloadError);
	...
	test('Refuses a custom header whose name is no token or whose value holds a line break', async () => {
		// A forged header name or a value with CR/LF in it would reach Mailgun's raw headers verbatim, so the message
