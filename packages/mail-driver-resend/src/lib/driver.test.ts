...
describe('MailDriverResend', () => {
	test('Sends and answers the id, throws on the SDK error value with it as the cause', async () => {
		...
		// The SDK reports failures as a value; the driver keeps it as the cause, so the caller can read the status code
		...
		expect(() => new MailDriverResend({ apiKey: '' })).toThrow(InvalidConfigError);
	...
	test('Sends attachments base64-encoded, a text body read into bytes first', async () => {
		// The mapper runs unmocked: the payload Resend receives must carry base64, never raw text or a local path
