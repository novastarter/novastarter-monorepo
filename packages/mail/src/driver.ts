...
export declare class MailDriver {
	 ...
	 * API — Resend's `GET /domains/{id}` — a full URL on one of the provider's own hosts, or the command name of an
	 * RPC-style SDK. A `{name}` in the path is filled from the parameter of that name; the other parameters are the
	 ...
	 * `call`.
	 *
	 * The `console` driver logs the request; `smtp`, `sendmail` and `file` have no API and leave it out.
	 *
	 ...
	 * @example
	 * ```ts
	 * const driver = useMail().location('resend');
