...
describe('MailDriverSmtp', () => {
	test('Builds the transport from the location options, with auth only when credentials are given', () => {
		// A host alone: neither port nor secure is forced, so nodemailer's own pair of defaults applies, no auth
		...
		expect(() => new MailDriverSmtp({ host: '' })).toThrow(InvalidConfigError);
		expect(() => new MailDriverSmtp({ host: '' })).toThrow(/"host"/);
	});

	test('Lets nodemailer derive the port from secure and secure from the port', () => {
		// `secure` alone: no port is forced, so nodemailer picks 465 instead of speaking TLS to 587
		...
		// Port 465 alone: no `secure: false` is forced, so nodemailer turns TLS on for the implicit-TLS port
		...
		// An explicit `secure: false` is still passed on: STARTTLS on 465 is unusual but the caller's call
