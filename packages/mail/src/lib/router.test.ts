...
const managerWith = (...names: string[]): MailManager => {
	// One driver, several locations: the chain is about names, not about transports
...
describe('resolveMailChain', () => {
	...
	test('Passes a rule made only of unknown names over for the next one', () => {
		...
		// A domain rule of typos is no rule: the category chain applies instead of an empty one failing every send
	...
	test('Falls back to every location without routes, and to nothing without locations', () => {
		// Empty routes: registration order is the chain, so one location needs no routes at all
