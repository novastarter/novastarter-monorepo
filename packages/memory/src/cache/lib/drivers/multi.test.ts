...
beforeEach(() => {
	...
	// Both levels are automocked, so the cache under test is the coordination between them and the bus
...
describe('constructor', () => {
	test('Creates local and redis cache handlers', async () => {
		// Each level gets its own config; without a ttl on L2 the L1 config goes through as it was
	...
	test('Creates a redis bus over the L2 connection and namespace', () => {
		// The invalidations travel on the same server and namespace as the values they concern
	...
	test('Gives L1 the ttl of L2 when it has none of its own', () => {
		// L2 expiry never reaches L1 — only writes publish invalidations — so an L1 without a ttl would serve a key L2
		// already lost, in the process that wrote it, until eviction
	...
	test('Keeps a shorter L1 ttl and refuses a longer one', () => {
		// A shorter L1 ttl is a valid choice: memory turns over faster than Redis
		...
		// A longer one would outlive L2, so it is refused at construction rather than served stale later
		expect(
			() => new CacheDriverMulti({ local: { maxKeys: 5, ttl: 61_000 }, redis: { ...mockRedisConfig, ttl: 60_000 } }),
		).toThrow(InvalidConfigError);

		// Without an L2 ttl any L1 ttl goes, since L2 keeps the key for good
...
describe('get', () => {
	test('Returns local value if exists', async () => {
		// A memory hit spares the Redis round trip
	...
	test('Returns redis value if local is undefined', async () => {
		// A miss falls through to L2; the value is not promoted into L1, only writes populate it
...
describe('set', () => {
	test('Writes L2 first and L1 only once L2 took the value', async () => {
		// Both levels get the value on a successful write
		...
		// And leaves no write counted as under way, so the next one starts clean
	...
	test('Keeps a write out of L1 when another process invalidated the key while it was in flight', async () => {
		// The L2 write is held pending, the way a reply still on the wire is
		let settle!: () => void;
		...
		// Another process's invalidation for the key arrives on the bus meanwhile: L1 is dropped, as always, and the
		// pending write is marked
		...
		// Once L2 answers, the value stays out of L1 — it may be older than what L2 holds now — while the invalidation
		// is still published for the others, and the key is no longer counted as under way
		settle();
		await write;
		...
		// The next write of the key starts clean and lands in L1 again
		await cache.set(mockKey, mockValue);
		expect(cache['local'].set).toHaveBeenCalledWith(mockKey, mockValue);
	});

	test('Marks every in-flight write when the invalidation carries no key, and none for another key', async () => {
		// Two writes of different keys pending at once
		const settles: (() => void)[] = [];
		...
		// An invalidation of a third key touches neither; a `clear` of everything marks both
		const onMessage = vi.mocked(cache['bus'].subscribe).mock.calls[0]![1];
	...
	test('Ignores its own invalidation for an in-flight write', async () => {
		// A message stamped with this process's id is one of its own, already accounted for; it must not keep a
...
describe('delete', () => {
	test('Deletes from L2 first, then L1, and tells the other processes', async () => {
		// Both levels drop the key on a successful delete
	...
	test('Keeps an in-flight set of the same key out of L1', async () => {
		// A `set` waits for its L2 reply; a concurrent `delete` of the same process removes the key from both levels,
		// but the set would still land in L1 afterwards — the sender skips its own invalidation messages — with L2
		// holding nothing and no further invalidation coming
		let settle!: () => void;
		vi.mocked(cache['redis'].set).mockReturnValueOnce(new Promise<void>((resolve) => (settle = resolve)));
		...
		// The delete ran, and the late write stayed out of L1
		expect(cache['redis'].delete).toHaveBeenCalledWith(mockKey);
		expect(cache['local'].delete).toHaveBeenCalledWith(mockKey);
		expect(cache['local'].set).not.toHaveBeenCalled();
		expect(cache['writing'].size).toBe(0);
...
describe('invalidation over the bus', () => {
	test('Subscribes to the cache channel at construction and publishes a stamped clear message on every write', async () => {
		// The contract the other processes rely on: channel, message shape and the origin that lets a sender skip its
		// own
		...
		// A delete announces the same key; a clear announces no key, which means every key
		await cache.delete(mockKey);
	...
	test('Drops the whole L1 and keeps in-flight writes out of it after the bus reconnected', async () => {
		// Invalidations sent while the subscriber was down are lost, so any L1 key may be stale after a reconnect
		...
		// The write in flight during the reconnect may predate a missed invalidation, so it skips L1
		settle();
		await pending;
...
describe('has', () => {
	test('Checks if redis has the value cached', async () => {
		// L2 is the source of truth: a key missing from this process's L1 may be cached elsewhere
...
describe('acquireLock', () => {
	test('Delegates to redis cache', async () => {
		// Only the Redis lock is visible to other processes; the handle is handed through untouched
...
describe('usingLock', () => {
	test('Delegates to redis cache', async () => {
		// The callback and its result travel through L2's `usingLock` unchanged
...
describe('clear', () => {
	test('Calls clear for both caches', async () => {
		// Both levels are emptied; the return value carries nothing
	...
	test('Keeps in-flight sets of this process out of L1', async () => {
		// Two writes of different keys wait for their L2 replies while a `clear` of the same process runs: like a
		// keyless invalidation of another process, it marks every in-flight write to skip L1 — the sender skips its own
		// messages, so without the mark each write would repopulate L1 after everything was cleared
		const settles: (() => void)[] = [];
		vi.mocked(cache['redis'].set).mockImplementation(() => new Promise<void>((resolve) => settles.push(resolve)));
		...
		// The clear ran, and neither late write reached L1
		expect(cache['local'].clear).toHaveBeenCalledOnce();
		expect(cache['local'].set).not.toHaveBeenCalled();
		expect(cache['writing'].size).toBe(0);
	});
...
describe('subscription', () => {
	test('Reports a failed subscription on the next write and subscribes again there', async () => {
		...
		// The first write tries again, fails again, and reports it without writing or publishing: a key put into L1 by
		// a process that receives no invalidations would go stale unseen. The automock records calls on the instance's
		// method, so the assertions go through the driver's own bus
		...
		// Once Redis is back the next write subscribes, publishes and succeeds; later writes reuse the subscription
		vi.mocked(BusDriverRedis.prototype.subscribe).mockResolvedValue(undefined);
...
describe('close', () => {
	test("Quits the bus, the one connection of the driver's own", async () => {
		// L2 runs on the caller's connection and L1 holds nothing, so the subscriber connection is all there is to quit
		await cache.close();
	...
	test('Refuses a write after close instead of filling an L1 nobody invalidates any more', async () => {
		// With the bus gone no invalidation can reach this process, so a write is refused before touching a level
		await cache.close();
...
describe('onMessageClear', () => {
	test('Ignores messages from self', async () => {
		// `set` and `delete` already updated L1 before publishing; dropping the key again would throw away fresh data
	...
	test('Clears specific key if provided in payload', async () => {
		// A keyed message drops that key alone
		await cache['onMessageClear']({
			type: 'clear',
	...
	test('Clears all keys if key is undefined', async () => {
		// A message without a key is another process's `clear()`, so everything local goes
		await cache['onMessageClear']({
			type: 'clear',
			origin: 'other-process',
		});
