...
beforeEach(() => {
	// Both ioredis and the Kv store are automocked, so the cache is built on doubles and every store method is a mock
	mockRedis = new Redis();
...
describe('get', () => {
	test('Returns result of store get', async () => {
		// The store reads, gunzips and parses; its answer comes back as is
...
describe('set', () => {
	test('Sets the value to the kv store', async () => {
		// The value is handed over as is; serializing, compressing and expiring it is the store's job
...
describe('delete', () => {
	test('Deletes key from kv store', async () => {
		// This cache holds no local copy, so the store's unlink is the whole operation
...
describe('has', () => {
	test('Returns result of kv store has', async () => {
		// The store's `EXISTS` round trip answers; the cache adds no bookkeeping of its own
...
describe('clear', () => {
	test('Clears the kv store', async () => {
		// The store scans and unlinks its namespace, which is the cache's namespace
		await cache.clear();
