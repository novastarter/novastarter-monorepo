...
beforeEach(() => {
	// The library is automocked: the tests drive its `consume` by hand and look at what the wrapper makes of it
	key = 'limiter-test-key';
	points = 5;
	limiter = new RateLimiterMemory({ points, duration: 15 });

	// Fake timers, since the `reset` date is computed from `Date.now()` and has to be asserted exactly
	vi.useFakeTimers();
});
...
test('Consumes point from rate-limiter', async () => {
	// The key goes to the library as it is; one point per call is the library's default
...
test('Rejects error as-is if error instance is given', async () => {
	// A real `Error` — a lost connection — is not a rate-limit hit and must reach the caller untouched
...
test('Rejects HitRateLimitError', async () => {
	// The clock is frozen so the expected `reset` can be computed the same way the wrapper does
	const systemTime = new Date('2023-12-04T00:00:00.000Z');
	vi.setSystemTime(systemTime);

	// The library rejects with a plain result object, not an `Error`, when the budget is spent
	...
	// The wrapper turns it into the error the transport layer knows, carrying the limit and the reset moment
...
test('Answers a key without expiry with a reset of now, not in the past', async () => {
	// The library reports `-1` for a key that never expires; added to the clock as it is, that would be a reset one
