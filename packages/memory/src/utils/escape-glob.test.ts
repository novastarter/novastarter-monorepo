...
test('Escapes every glob operator and leaves the rest alone', () => {
	// Each operator gets a backslash, so `MATCH` reads it as the literal character
	...
	// Text without operators is unchanged, so the common namespace produces the pattern it always did
