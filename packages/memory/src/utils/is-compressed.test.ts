...
test('Returns false if byte length is less than 19', () => {
	// A gzip stream is at least 19 bytes; anything shorter cannot be one, whatever it starts with
...
test('Returns false if first byte does not match magic number', () => {
	// The magic number is `1f 8b`; a wrong first byte rules gzip out even with the rest in place
...
test('Returns false if second byte does not match magic number', () => {
	// Both magic bytes are needed; the second one alone wrong is enough to say no
...
test('Returns false if third byte does not compression flag', () => {
	// The third byte is the method; gzip always uses deflate (`08`), so another value is not a stream `decompress`
	// reads
...
test('Returns true if third byte does not compression flag', () => {
	// Magic number, deflate method and enough length: a stream `decompress` can read
