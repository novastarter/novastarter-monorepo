...
describe('toMetadata', () => {
	test('Strings stay, scalars are written out, nested values become JSON', () => {
		// 1. One value of every JSON kind proves each is written the way the kit's shapes can carry it
		...
		// 2. Paddle sends `null` for custom data never set; the kit's metadata is then empty, not absent
