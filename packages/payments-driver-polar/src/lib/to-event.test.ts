...
describe('toEvent', () => {
	test('Maps the events the kit acts on and drops the rest', () => {
		// A succeeded checkout is a purchase, with the subscription Polar created and the metadata it copied over
		...
		// A checkout still open is not a purchase yet
		expect(toEvent(parsed('checkout.updated.open'), 'msg_0')).toBeNull();

		// The subscription lifecycle: created, the catch-all update, and revoked as the end of the subscription
		...
		// A `subscription.updated` carrying the canceled status is the revocation's echo — a retried or late one
		...
		// The specific subscription events repeat what `subscription.updated` already said
...
describe('toCompletedCheckout', () => {
	test('Keeps the shape for a checkout the SDK types without a customer or a subscription', () => {
		// The SDK's optional fields become the kit's empty id and `null`, so the event still has every key
...
describe('deliveryIdOf', () => {
	test('Reads the Standard Webhooks id and answers nothing without it', () => {
		// The header is the one value Polar keeps stable across retries, so it is what the kit deduplicates on
