...
describe('toMetadata', () => {
	test('Writes every value out as a string', () => {
		// Numbers and booleans are written the way `String()` writes them, so a consumer reads one type
		expect(toMetadata({ a: 'x', b: 2, c: true })).toStrictEqual({ a: 'x', b: '2', c: 'true' });

		// Polar answers `null` for no metadata; the kit's shape is never nullable
