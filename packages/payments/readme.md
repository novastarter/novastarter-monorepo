...
built so far at shutdown and keeps the registrations. `location()` without a name answers with `default`
(`DEFAULT_LOCATION` of `@novastarter/utils`), the location a deployment with one provider registers and the one
`handleWebhook()` verifies against.
...
```ts
usePayments().registerLocation('stripe', {
	driver: 'stripe',
	options: { secretKey: env.STRIPE_SECRET_KEY, webhookSecret: env.STRIPE_WEBHOOK_SECRET },
	call: { headers: { 'Stripe-Account': 'acct_123' } },
});

const { status, headers, data } = await usePayments().location('stripe').call!('GET /v1/customers/{id}', {
	id: 'cus_123',
});
```
