...
for an `https:` target — the click link; `data` carries the custom pairs with the click target under `url`. The result
is FCM's message name as the id.
...
| `ttl`            | —        | Seconds FCM keeps a message for an offline device; FCM's four weeks unless given.         |
