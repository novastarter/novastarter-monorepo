...
describe('describeError', () => {
	test('Reports a 404 or 410 as a gone target naming the status and endpoint', () => {
		// A 410 is Chrome's answer for an unsubscribed browser: the error the caller deletes the subscription on, the
		...
		// A 404 is what Firefox and Safari answer for the same thing, so it is as gone as a 410
	...
	test('Names any other status and the trimmed body, keeping the library error as the cause', () => {
		// The body is the push service's own explanation, so it is kept; the cause keeps the status code reachable
		...
		// An empty body adds no trailing colon
	...
	test('Prefixes anything that is not a service answer and passes it on as the cause', () => {
		// A network failure never reached the service, so there is no status to report: only the message
