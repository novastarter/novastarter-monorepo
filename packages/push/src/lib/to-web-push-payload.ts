...
export const toWebPushPayload = (message: PushMessage): WebPushPayload => {
	// Every optional field stays out of the JSON, which is capped at 4 KB, and the click target rides in `data.url` since
	// `showNotification()` keeps `data` on the notification for the worker to read back in `notificationclick`
	return {
		title: message.title,
