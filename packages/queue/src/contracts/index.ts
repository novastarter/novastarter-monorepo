...
export const registerJob = <Contract extends JobContract>(contract: Contract): Contract => {
	// A second contract under one name is refused rather than replacing the first: `enqueue()` and the worker
	...
	// Kept by name, which is what a job carries through the queue; answered back for the one-line export
...
export const getJobContract = (name: string): JobContract => {
	// The name is whatever arrived — from a caller, or from BullMQ on the worker side
	const contract = _contracts.get(name);

	// An unknown name is thrown, not answered with `undefined`: a producer and a worker that disagree on the
