...
describe('defineJob', () => {
	test('Splits the name into queue and action and applies the default options', () => {
		// The name becomes queue and action; the options are the package defaults unless overridden
	...
	test.each(['send', 'mail.send.now', 'Mail.send', 'mail_x.send', '.send', 'mail.', '1mail.send'])(
		'Refuses the name %s',
		(name) => {
			// Every shape outside `<queue>.<action>` is refused before a contract exists
	...
	test('Refuses a timeout a timer cannot hold, where the contract is written rather than at every run', () => {
		// Negative, `NaN` and overlong timeouts are refused at definition time
		...
		// The message names the offending value, so a bad timeout is findable among a list of contracts
		...
		// The upper bound itself is a delay a timer can hold
	...
	test('Parses a payload, applying defaults', () => {
		// Defaults are applied at parse time, so the handler always sees a complete payload
		const job = defineJob({ name: 'mail.send', schema });
	...
	test('Reports every issue of an invalid payload at once', () => {
		// One thrown error carries every issue, so a caller learns all that is wrong in a single round
		const job = defineJob({ name: 'mail.send', schema });
