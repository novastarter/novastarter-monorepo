...
export function durationToCron(duration: number): string {
	// A random phase inside the minute; `randomInt` takes an exclusive upper bound, hence 60 for 0–59
	const second = randomInt(0, 60);
	const minute = randomInt(0, 60);

	// Whole hours only, and only the intervals that divide a day evenly, so the rule repeats identically every day
