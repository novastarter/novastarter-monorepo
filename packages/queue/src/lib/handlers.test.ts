...
afterEach(() => {
	// Handlers are per-application state: whatever one test registered may not leak into the next
	_handlers.clear();
});

describe('registerJobHandlers', () => {
	test('Registers every handler given, keyed by job name', () => {
		// Every entry lands under its job name, so the worker finds the handler of a contract by name
	...
	test('Refuses a second handler for the same job', () => {
		// The first registration wins and says so: a second handler for the same job would leave the winner
		// ambiguous
