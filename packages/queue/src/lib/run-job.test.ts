...
afterEach(() => {
	// Contract and handler of a test are removed together, so the next one starts from the same empty registry
...
describe('runJob', () => {
	test('parses the payload with the contract and hands it to the registered handler', async () => {
		...
		// The handler got the parsed payload — coerced, defaults applied — and the run's context
	...
	test('refuses an unknown job, a job without a handler, and a payload off the contract', async () => {
		// An unknown job fails before anything runs
		await expect(runJob('nope.run', {}, context)).rejects.toThrow('Job "nope.run" is not registered');

		// A registered job without a handler fails just as clearly
		...
		// A payload off the schema is refused with the structured error, before the handler ever sees it
