...
export const startSchedules = (options: StartSchedulesOptions): RunningSchedules => {
	...
	for (const schedule of getSchedules(env)) {
		// A switched-off schedule is the deployment's choice, worth a debug line and nothing more
		if (!schedule.enabled) {
			logger.debug(`Schedule of "${schedule.job}" is disabled`);
			continue;
		}

		// A bad rule is a configuration error of one schedule, not of the worker
		if (!validateCron(schedule.cron)) {
			logger.error(`Schedule of "${schedule.job}" has an invalid cron rule "${schedule.cron}"; skipped`);
			continue;
		}

		// Two schedules resolving to one rule for one job — a string and a function, say, which `registerSchedule`
		...
		// A tick that fails to enqueue is reported and the schedule
		...
		running.push(
			scheduleSynchronizedJob(
				...
				schedule.cron,
				async (fireDate) => {
					// The payload goes as registered; `enqueue()` validates it against the contract on every tick
					const job = await options.enqueue(schedule.job, schedule.payload as JobInputOf<JobName>);

					// At debug level, so a deployment can be checked for which instance won which tick
