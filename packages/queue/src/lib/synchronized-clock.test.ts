...
beforeEach(() => {
	// Fake timers pin "now", so the clock readings of a test are exact
...
describe('SynchronizedClock', () => {
	test('Lets only the first instance advance the clock to a time', async () => {
		// Two instances race on one store: the first write wins, a later one cannot repeat the reading or move
		// the clock backwards
		...
		// Clocks of different ids do not share a reading
		expect(await new SynchronizedClock('test.ping', kv).set(1_000)).toBe(true);

		// Forgetting the clock lets the next write win again
		await first.reset();
