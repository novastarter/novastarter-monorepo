...
export const registerSchedule = <Name extends JobName>(schedule: Schedule<Name>): void => {
	// 1. Job and rule together are the identity of a schedule: its synchronised clock is keyed by both, so two
	...
	// 2. Registration order is start order, which the logs of `startSchedules()` follow
...
export const getSchedules = (env: ScheduleEnv): ResolvedSchedule[] => {
	// 1. The rule and the switch are read now, against the environment of this process, so a schedule can be turned
