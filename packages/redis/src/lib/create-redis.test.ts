...
afterEach(() => {
	...
	// A test that registered a process logger must not leak it into the next one
	useLogger.reset();
});

test("Takes a connection URL apart with ioredis's own parser and lays the overrides over what it carries", () => {
	// Host, port, credentials, database and query parameters, exactly as ioredis reads them (ports and the
	...
	// An override wins over the URL's query, where ioredis itself would keep the URL's value
	...
	// `rediss://` turns TLS on, as it does when ioredis reads the string itself — its parser alone leaves it off —
	...
	// Every other form ioredis accepts keeps its meaning: an IPv6 literal, a scheme-less host and port, a socket
...
test('Reports a client error through the given logger instead of letting it crash the process', () => {
	// A caller's own logger receives the connection errors of the client it created; an `error` event with no
	...
	// The client registered an `error` listener; invoking it — what ioredis does on every failed reconnect of a
...
test('Reports a client error through the process logger when none is given', () => {
	// No logger of its own: the process-wide one takes the line — the path every RedisManager location's client
	...
	// Emitting the error — ioredis's plain `emit`, the path that throws with no listener — must log and not throw
	const listener = vi.mocked(redis.on).mock.calls.find(([event]) => event === 'error')![1] as (error: Error) => void;
