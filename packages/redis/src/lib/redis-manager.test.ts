...
test('Opens the client of a location on first use and hands it out by name from then on', () => {
	// Registering a location opens nothing; the client comes on first use
	...
	// The first use opens the client once, with the registered config; every later use hands the same one out
...
test('Answers default without a name and throws for an unknown location', () => {
	// With no default registered, a bare lookup has nothing to answer and an unknown name throws
	const client = {} as Redis;
	vi.mocked(createRedis).mockReturnValue(client);

	const manager = new RedisManager();

	expect(() => manager.location('jobs')).toThrowErrorMatchingInlineSnapshot(
		`[NovastarterError: Invalid config. Location "jobs" doesn't exist; register it with registerLocation() before using it.]`,
	);

	// The default location answers a bare `location()`; an unknown name still throws and reports its absence
...
test('Quits every opened client on close, leaving the registrations in place', async () => {
	// Two locations registered, only the default one used: the close quits exactly the opened clients
	...
	// Registrations survive the close; only the instantiated clients are gone
...
test('Disconnects a client that never reached ready instead of waiting for a quit', async () => {
	// A client that is still connecting, or already ended, has no server to quit: `quit` would reconnect forever
