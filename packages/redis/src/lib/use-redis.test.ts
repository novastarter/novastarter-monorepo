...
describe('useRedis', () => {
	test('Creates one empty manager and keeps it across calls', () => {
		// 1. The first call builds an empty manager; every later call answers the same instance
	...
	test('Builds a fresh manager after reset()', () => {
		// 1. A fresh manager knows nothing of the old one: its locations are gone
		const first = useRedis();
