 ...
 *
 * @param tree - Relative paths mapped to file contents.
 */
...
/**
 * Builds a minimal tab-indented `package.json` body for a fixture package.
 *
 * @param name - Package name.
 * @param version - Package version.
 * @returns The JSON text to seed into the workspace.
 */
...
describe('readProject', () => {
	test('should read the manifest and resolve both root dirs', async () => {
		await seed({ 'packages/a/package.json': manifest('@novastarter/a', '2.3.4') });
		...
		expect(project).toMatchObject({
			...
			manifest: { name: '@novastarter/a', version: '2.3.4' },
		});
	});
