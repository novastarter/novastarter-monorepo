...
export async function processPackages(): Promise<{
	...
	// 1. Detect the packages `changesets` bumped and get rid of the changelog files it generated
	...
	// 2. Work out the headline version before bumping anything, as the bumps depend on its prerelease state
	const { mainVersion, manualMainVersion, isPrerelease, prereleaseId } = getVersionInfo();

	// 3. A forced version overrides whatever `changesets` gave the main package, dependents included
	if (manualMainVersion && config.mainPackage) {
		await bumpPackage(config.mainPackage, mainVersion, true);
	}

	// 4. Linked packages follow their trigger with a patch bump unless `changesets` bumped them already
	...
	// 5. The main package and the untyped packages are not listed under the published versions
	return {
		mainVersion,
		isPrerelease,
		prereleaseId,
	...
	function transformGraph(graph: Record<string, PackageNode<Project>>) {
		...
		for (const dependentNodeId of Object.keys(graph)) {
			...
			// 1. Graph nodes are keyed by directory, so every dependency is mapped back to its package name
			for (const dependencyNodeId of dependentPackage.dependencies) {
				...
				// 2. Register the current package as a dependent of each of its dependencies
