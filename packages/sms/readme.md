...
```ts
useSms().registerLocation('twilio', {
	driver: 'twilio',
	options: { accountSid: env.TWILIO_ACCOUNT_SID, authToken: env.TWILIO_AUTH_TOKEN },
	call: { timeout: 10_000 },
});

const { status, headers, data } = await useSms().location('twilio').call!(
