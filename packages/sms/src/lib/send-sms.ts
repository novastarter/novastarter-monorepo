...
export const sendSms = async (message: SmsMessage, options: SmsSendOptions = {}): Promise<SmsSendResult | null> => {
	...
	for (const location of chain) {
		...
		try {
			...
			// 6. pino takes a non-object first argument as the message, so a driver rejecting with a string would replace
	...
	// 7. Nobody took it: the reason is the limit when that is all that stood in the way, the last failure otherwise
