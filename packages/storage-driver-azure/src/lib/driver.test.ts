...
beforeEach(() => {
	...
	// The SDK module is mocked above, so constructing the driver only records calls and never opens a socket
	...
	// The private path resolver gets a lookup table, so assertions can match exact blob names without depending on the
	// mocked `joinPath`
...
describe('#constructor', () => {
	test.each([
		...
		// The SDK would only fail on the first request; the driver names the missing option at construction instead
		expect(
			() =>
		...
		).toThrowError(new InvalidConfigError({ reason: `The azure storage driver needs ${expected}` }));
	});

	test('Refuses a chunk size above the append-block limit when resumable uploads are on', () => {
		// Azure rejects appended blocks above 100 MiB, so a larger chunk would only fail mid-upload
		expect(
			() =>
				new StorageDriverAzure({
					containerName: sample.config.containerName,
		...
		).toThrowError(new InvalidConfigError({ reason: 'The azure storage driver got a "tus.chunkSize" above 100 MiB' }));
	});

	test.each([[-1], [0], [Number.NaN]])(
		'Refuses a non-positive chunk size of %s when resumable uploads are on',
		(chunkSize) => {
			// A zero, negative or NaN size would be kept as the per-chunk bound and make every arriving chunk fail; NaN
			// slips through every comparison, which is why the check is written as `!(size > 0)`
			expect(
				() =>
					new StorageDriverAzure({
						containerName: sample.config.containerName,
			...
			).toThrowError(new InvalidConfigError({ reason: 'The azure storage driver got a "tus.chunkSize" below 1 byte' }));
		},
	);

	test('Creates signed credentials', () => {
		// The shared driver from `beforeEach` already ran the constructor, so the credential call is recorded; the
	...
	test('Creates blob service client and sets the client', () => {
		// Hand the SDK fixed instances, so the assertions can follow the wiring by identity instead of by shape
		...
		// Without an endpoint the driver must derive the public one from the account name and sign it with the
		...
		// The container handle has to come from that same service client, or requests would go to another account
	...
	describe('Allows overriding endpoint with optional setting', () => {
		test('Creates blob service client and sets the client', () => {
			...
			// A custom endpoint must reach the SDK verbatim, or emulators and sovereign clouds would still be routed to
			...
			// The container handle has to come from that same service client, or requests would go to another host
			expect(mockBlobServiceClient.getContainerClient).toHaveBeenCalledWith(sample.config.containerName);
			expect(driver.client).toBe(mockContainerClient);
		});
	});
	...
	test('Defaults root path to empty string', () => {
		// The shared driver was built without a root; an empty string rather than `undefined` keeps `joinPath` from
		// producing blob names that start with "undefined/"
		expect(driver['root']).toBe('');
	});

	test('Normalizes config path when root is given', () => {
		// `confinePath` is auto-mocked, so the call alone shows the root goes through the same resolution as every key:
		// a leading slash or `..` in the root would otherwise end up inside the blob names
		new StorageDriverAzure({
			containerName: sample.config.containerName,
			accountName: sample.config.accountName,
			accountKey: sample.config.accountKey,
...
describe('#read', () => {
	...
	beforeEach(async () => {
		// Resolve a stream by default, so the range tests only have to assert on the arguments handed to the SDK
	...
	test('Throws StorageFileNotFoundError when the blob is missing, rethrows anything else', async () => {
		// A 404 is the error every backend shares, so callers can tell a missing blob from any other failure
		...
		// A denied read says nothing about the blob, so it must keep its own identity instead of becoming "not found"
	...
	test('Uses blobClient at full path', async () => {
		// The blob name must be the resolved one, not the caller path, or the root prefix would be lost
	...
	test('Calls download with undefined undefined when no range is passed', async () => {
		// Without a range the SDK gets neither offset nor count, which is how it reads the whole blob
		await driver.read(sample.path.input);
	...
	test('Calls download with offset if start range is provided', async () => {
		// A start alone becomes the offset; the count stays undefined so the rest of the blob is read
	...
	test('Calls download with count if end range is provided', async () => {
		// An end alone counts from byte zero; the bound is inclusive, hence one more than the end
	...
	test('Calls download with offset and count if start and end ranges are provided', async () => {
		// The SDK takes offset and count, so the closed range has to be converted rather than passed through
	...
	test('Throws error when no readable stream is returned', async () => {
		// `readableStreamBody` is only set in Node; without it there is nothing to hand back, and a silent `undefined`
		// would crash the caller further away from the cause
		...
		await expect(driver.read(sample.path.input)).rejects.toThrowError(
			`The azure storage driver got no stream for file "${sample.path.input}"`,
		);
	});
});
...
describe('#write', () => {
	...
	beforeEach(() => {
		// Record the upload call only; the mocked SDK never consumes the stream, so no data has to be written to it
	...
	test('Gets BlockBlobClient for file path', async () => {
		// The block blob must be addressed by the resolved name, or the write lands outside the root
	...
	test('Uploads stream through uploadStream', async () => {
		// Buffer size and concurrency stay at the SDK defaults; the only header set is a generic content type, so the
		// blob is never served without one
		await driver.write(sample.path.input, sample.stream);
	...
	test('Allows optional mime type to be set', async () => {
		// A caller-supplied type has to reach the blob's `Content-Type`, since that is what the service serves it with
...
describe('#delete', () => {
	...
	beforeEach(() => {
		// Resolve `true` as the SDK does for a blob that was present; the driver ignores the flag either way
	...
	test('Uses blobClient at full path', async () => {
		// The blob name must be the resolved one, or a delete could remove a blob outside the root
	...
	test('Returns delete result', async () => {
		// `deleteIfExists` rather than `delete`, so removing a blob that is already gone is a no-op, not a 404
		await driver.delete(sample.path.input);
...
describe('#stat', () => {
	beforeEach(() => {
		// Answer the HEAD request with the fixture's size and date, so the mapping test has known values to compare
	...
	test('Uses blobClient at full path', async () => {
		// The blob name must be the resolved one, or the metadata would describe a blob outside the root
	...
	test('Returns contentLength/lastModified as size/modified from getProperties', async () => {
		// Only the two fields every backend shares are returned, under the names the `Stat` contract uses
	...
	test('Maps a 404 to the kit error', async () => {
		// The SDK's `RestError` carries the HTTP status as `statusCode`; 404 becomes the error every backend shares
	...
	test('Rethrows any other SDK error', async () => {
		// A 403 says nothing about whether the blob exists, so it must not be reported as "not found"
	...
	test('Refuses a properties response missing size or modification time', async () => {
		// Both fields are optional in the SDK's types; answering `undefined` under the non-optional `Stat` type would
		// fail far from its cause, so a broken response is refused with the path named
		Object.assign(driver, {
			client: {
		...
		await expect(driver.stat(sample.path.input)).rejects.toThrowError(
			`The azure storage driver got no size or modified time for file "${sample.path.input}"`,
		);
	});
});
...
describe('#exists', () => {
	...
	beforeEach(() => {
		// Answer `true` by default, so the pass-through test can tell the SDK's answer from a hard-coded one
	...
	test('Uses blobClient at full path', async () => {
		// The blob name must be the resolved one, or the check would report on a blob outside the root
	...
	test('Returns exists result', async () => {
		// The SDK already does the HEAD request and the 404 check, so its answer is returned untouched
	...
	test('Throws if the lookup failed', async () => {
		// The SDK only answers `false` for a missing blob, so a failed lookup has to keep travelling; reporting it as a
...
describe('#move', () => {
	...
	beforeEach(() => {
		// Only the delete step runs against the SDK here; `copy` is stubbed so the move is checked in isolation
	...
	test('Calls #copy with src and dest', async () => {
		// Blob Storage has no rename, so the move must reuse the server-side copy with the caller's paths
	...
	test('Deletes src file after copy is completed', async () => {
		// The source is removed by its resolved name and exactly once; a second delete would be a wasted request
		await driver.move(sample.path.src, sample.path.dest);
...
describe('#copy', () => {
	...
	beforeEach(() => {
		// The poller resolves at once, so `copy` returns without the test having to drive a pending server-side copy
		...
		// The source is a block blob client (only its `url` is read); the target is a plain blob client, since it may
		// hold an append blob from a TUS upload
		mockBlockBlobClient = vi.fn().mockReturnValue({
			url: mockUrl,
		});
	...
	test('Gets clients for src and dest', async () => {
		// Both names are resolved, so neither end of the copy can escape the root
	...
	test('Removes a target of another blob type and copies again', async () => {
		// Azure copies onto an existing blob only of the source's type; an append blob from a TUS upload is refused as
		// the target of a block blob, and the other way round
		...
		// The mismatched target is deleted before the second copy, which then succeeds
	...
	test('Rethrows any other conflict without touching the target', async () => {
		// A conflict other than `InvalidBlobType`, such as a copy still pending on the target, is not recoverable
		...
		// The target is kept, so the existing content is not lost
		expect(mockTargetDeleteIfExists).not.toHaveBeenCalled();
	});

	test('Calls beginCopyFromUrl with source url', async () => {
		// The copy is started on the target from the source's URL, which is the only way the service copies blobs
		await driver.copy(sample.path.src, sample.path.dest);
	...
	test('Waits for the polling to be done', async () => {
		// The copy runs asynchronously on the server, so returning before the poller finishes would let a `move` delete
		// the source while the copy is still pending
		await driver.copy(sample.path.src, sample.path.dest);
...
describe('#list', () => {
	...
	beforeEach(() => {
		// An empty page by default; a plain array stands in for the SDK's paged iterator, since `for await` accepts
		// either
	...
	test('Uses listBlobsFlat at default empty path', async () => {
		// With no prefix the whole root is listed; the resolver still runs so the root itself becomes the prefix
	...
	test('Allows for optional prefix', async () => {
		// The prefix is resolved like any key, so a listing under `media` cannot spill into a sibling folder
	...
	test('Returns blob.name for each returned blob', async () => {
		// With an empty root there is nothing to strip, so the blob name comes back as the relative path unchanged
	...
	test('Skips the staging blobs of resumable uploads in flight', async () => {
		// A `<name>.<id>.tmp` blob holds a partial upload, not an object a caller stored
		const mockFile = randFilePath();
	...
	test('Skips folder placeholder blobs ending in a slash', async () => {
		// ADLS Gen2 and several upload tools create zero-byte `folder/` markers; a caller that pipes `list()` into
...
describe('#writeChunk', () => {
	...
	test('Pins the append to the offset the chunk starts at', async () => {
		// Append blobs append at the current end unconditionally; pinning the position makes a resent chunk fail with
		// 412 instead of appending its bytes a second time and corrupting the upload
	...
	test('Refuses a context without a valid staging id before any request', async () => {
		// A context that lost its id, or carries a crafted one, must not address the target or another blob
	...
	test('Refuses a chunk above the configured size', async () => {
		// The TUS server agreed to send at most the configured size per request; a larger chunk is refused before the
		// service would reject the append mid-upload
		...
		await expect(
			...
			new InvalidPayloadError({
				reason: `The chunk of ${Buffer.byteLength(sample.text)} bytes exceeds the chunk size limit of 1 bytes`,
			}),
		);
	...
	test('Stops consuming the stream once the chunk crosses the configured size', async () => {
		// The bound is enforced while the chunk is still arriving, so a chunk that never ends must be refused rather
		...
		// Only the first pull is needed to cross the bound, and the stream is destroyed rather than drained, so the
...
describe('#call', () => {
	...
	beforeEach(() => {
		// The real deadline, a SAS of known content, and a `fetch` that records the request
	...
	afterEach(() => {
		// The global `fetch` is the real one again for the other tests
		vi.unstubAllGlobals();
	});

	test('Requests the account endpoint with the parameters and the SAS in the query', async () => {
		// `{container}` becomes the container; the SAS rides in the query, the API version in a header
	...
	test('Fills a placeholder from the parameters, encoded, and does not send that parameter again', async () => {
		// `{blob}` takes the `blob` parameter; `/` in it cannot reshape the path, and it is not in the query
	...
	test('Refuses a placeholder nobody filled before anything is sent', async () => {
		// Sent, `{blob}` would reach Azure as `%7Bblob%7D`
	...
	test('Answers with the status, the headers lower-cased and the body', async () => {
		// A HEAD's properties arrive in headers only
	...
	test('Signs a short-lived blob SAS with the account credential', async () => {
		// Blob service only, the resource types `call()` addresses, a lifetime of minutes — and read, delete, list and
		// tag only, never write or create, so a SAS that leaks cannot change anything in the account
	...
	test.each(['PUT /', 'POST /{container}', 'PATCH /'])(
		...
		async (method) => {
			// A write's body is the SDK's job; the caller is pointed at the client
	...
	test('Sends the headers of the caller over its own', async () => {
		// The caller's headers go on top of the API version
	...
	test('Refuses a full URL on a foreign host before anything is sent', async () => {
		// A SAS for the account must not travel to another party
	...
	test('Turns an error status into a ProviderCallError without the SAS or the account key', async () => {
		// Azure quoting the signature in its answer must not put it into the error
	...
	test('Turns a 429 into a HitRateLimitError', async () => {
		// Azure asking to slow down becomes the kit's rate-limit error
	...
	test('Gives up at the timeout of the caller', async () => {
		// A request that never answers; the error is matched by shape, since `@novastarter/utils` is mocked here
	...
	test('Follows a redirect to another host without the SAS or its headers', async () => {
		// The SAS rides in the first URL only, so the next hop carries neither it nor the API version
		fetchMock
	...
	test('Reports a failure to reach Azure without its cause, which may quote the signed URL', async () => {
		// A network error naming the URL, SAS included, is replaced by one that names only its code
	...
	test('Counts the reading of a slow answer against the timeout', async () => {
		// Headers in time, a body that never ends: the deadline still ends the call
		fetchMock.mockImplementation(
			async (_url: string, init: RequestInit) =>
				new Response(
					new ReadableStream({
						start(controller) {
							// The body errors with the abort reason, as a real one does when its request is aborted
...
describe('#tusExtensions', () => {
	test('Advertises creation, termination and expiration', () => {
		// Only the extensions the chunked-upload methods back; checksum and concatenation are left out because an
...
describe('#createChunkedUpload', () => {
	...
	test('Creates an empty staging append blob and keeps its id in the context', async () => {
		...
		// The id is a fresh random one, replacing whatever the client sent under the key
	...
	test('Leaves a blob already at the path untouched', async () => {
		// Only the staging blob is created, so an existing target keeps its content while the upload runs and after it
		// is abandoned
	...
	test('Gives each upload of the same path its own staging blob', async () => {
		// Two concurrent uploads must never append to one blob
...
describe('#finishChunkedUpload', () => {
	...
	beforeEach(() => {
		// The staging blob only has its URL read and is deleted at the end; the copy is started on the target
	...
	test('Copies the staging blob over the target, waits for the copy, then removes the staging blob', async () => {
		// A target of the same type accepts the copy on the first try
		...
		// The staging blob goes only after the copy is done, or the copy would lose its source
	...
	test('Removes a target of another blob type and copies again', async () => {
		// Azure copies onto an existing blob only of the source's type; a block blob from `write()` is refused
		mockBeginCopyFromUrl.mockRejectedValueOnce(
			Object.assign(new Error('invalid blob type'), { statusCode: 409, code: 'InvalidBlobType' }),
		);
	...
	test('Keeps the staging blob and rethrows when the copy after removing the target fails', async () => {
		// The first copy is refused for the blob type, and the second one fails after the target is deleted
		...
		// The old target is gone, but the staging blob survives so a retried finish can still complete the upload
		expect(mockTargetDeleteIfExists).toHaveBeenCalledOnce();
		expect(mockBeginCopyFromUrl).toHaveBeenCalledTimes(2);
	...
	test('Rethrows any other conflict without touching the target or the staging blob', async () => {
		// A conflict other than `InvalidBlobType`, such as a copy still pending on the target, is not recoverable here
		const error = Object.assign(new Error('pending copy'), { statusCode: 409, code: 'PendingCopyOperation' });
		mockBeginCopyFromUrl.mockRejectedValueOnce(error);

		await expect(driver.finishChunkedUpload(sample.path.input, stagedContext())).rejects.toBe(error);

		// Neither blob is deleted, so the target is not lost and the finish can be retried
		expect(mockTargetDeleteIfExists).not.toHaveBeenCalled();
		expect(mockStagingDeleteIfExists).not.toHaveBeenCalled();
	});

	test('Throws StorageFileNotFoundError when the staging blob is gone', async () => {
		// A 404 on the copy means the source is missing: the upload is unknown or was already finished
	...
	test('Throws StorageFileNotFoundError for a context without a staging id', async () => {
		// Without an id there is no staging blob to name, so the driver fails before any request
		await expect(
...
describe('#deleteChunkedUpload', () => {
	...
	beforeEach(() => {
		// Both kinds of client are mocked, so a test can prove the target is never deleted
		mockStagingDeleteIfExists = vi.fn().mockResolvedValue(undefined);
		mockTargetDeleteIfExists = vi.fn().mockResolvedValue(undefined);
	...
	test('Deletes only the staging blob and leaves the target alone', async () => {
		// An aborted upload never wrote the target, so a file stored there before survives the termination
	...
	test('Throws StorageFileNotFoundError for a context without a staging id', async () => {
		// Without an id there is no staging blob to name, and the target must never be deleted in its place
		await expect(
