...
describe.each([
	...
	test('Holds only lower-case, dot-prefixed extensions', () => {
		// The driver compares a lower-cased `extname`, which keeps the dot, against these lists; an entry in another
	...
	test('Holds every extension once', () => {
		// A duplicate is harmless to the lookup but hides a copy-paste slip when the list is edited
		expect(new Set(extensions).size).toBe(extensions.length);
	});
});

test('IMAGE_EXTENSIONS and VIDEO_EXTENSIONS do not overlap', () => {
	// The image list is checked first, so an extension on both lists would never reach the video endpoint
...
test('MINIMUM_CHUNK_SIZE is 5 MiB', () => {
	// Cloudinary rejects chunks below 5 MB; the constant gates the TUS chunk size at construction
