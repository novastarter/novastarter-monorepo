...
test('Returns an empty string for an empty object', () => {
	// No entries means no body at all; a stray separator would be read by the server as an empty parameter
	expect(toFormUrlEncoded({})).toBe('');
});

test('Serializes entries as key=value pairs joined by ampersands, in insertion order', () => {
	// A request body keeps insertion order by default, so callers control which parameter Cloudinary sees first
	expect(
		toFormUrlEncoded({
			second: 'two',
			first: 'one',
...
test('Sorts entries alphabetically by key when the sort option is enabled', () => {
	// Sorting produces a canonical form, so two payloads with the same entries serialize to the same string
	expect(
		toFormUrlEncoded(
			{
				b_key: 'second',
...
test('Keeps slashes in values unescaped so folder paths survive', () => {
	// Folder paths are part of a public id; leaving the slash verbatim keeps the body readable in request logs
...
test('Percent-encodes a plus sign so it is not read as a space', () => {
	// A form body decodes `+` as a space, so a public id such as `c++.zip` must go out as `%2B`; decoding the whole
...
test('Percent-encodes ampersand, equals and percent so a value cannot split the parameter list', () => {
	// An unescaped `&` or `=` in a value starts a new parameter on the server, and a bare `%` is a broken escape
...
test('Round-trips every value through a form-body parser unchanged', () => {
	// What matters is what the server reconstructs: parsing the body back must yield the raw values, since the
