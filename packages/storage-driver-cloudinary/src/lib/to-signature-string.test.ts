...
test('Returns an empty string for an empty object', () => {
	// With nothing to sign only the API secret goes into the digest; a stray separator would change the hash
	expect(toSignatureString({})).toBe('');
});

test('Sorts entries alphabetically by key', () => {
	// Cloudinary rebuilds the signed string in alphabetical key order, so insertion order must not leak into it
	expect(
		toSignatureString({
			b_key: 'second',
			c_key: 'third',
...
test('Preserves spaces in values instead of encoding them as plus signs', () => {
	// Cloudinary signs the raw values it receives, so a space encoded as `+` would produce a digest it never matches
...
test('Emits values verbatim without any URL-encoding', () => {
	// Every reserved character stays as it is for the same reason: the digest is computed over the raw values
