...
test('toListPrefix asks for a folder with a trailing slash and leaves a partial key alone', () => {
	// The whole root is `root/`, so `media-archive/…` is not matched by `media`
	expect(toListPrefix('media', '')).toBe('media/');
	expect(toListPrefix('media/avatars', 'avatars/')).toBe('media/avatars/');

	// A partial key stays a partial key; the whole bucket is the empty string
...
test('toListPrefix treats a prefix that resolves to a folder as that folder, not as a partial key', () => {
	// `.`, `..` and `avatars/..` resolve to the root, so they list `media/` and cannot reach `media-archive/…`
	expect(toListPrefix('media', '.')).toBe('media/');
	expect(toListPrefix('media', '..')).toBe('media/');
	expect(toListPrefix('media', 'avatars/..')).toBe('media/');
	expect(toListPrefix('media', '/')).toBe('media/');

	// A `.` segment at the end names the folder before it; a backslash is a separator like a slash
	expect(toListPrefix('media/avatars', 'avatars/.')).toBe('media/avatars/');
	expect(toListPrefix('media/avatars', 'avatars\\')).toBe('media/avatars/');

	// A `..` in the middle only changes which key is partial; a segment of three dots is a name, not a folder
	expect(toListPrefix('media/a/c', 'a/b/../c')).toBe('media/a/c');
	expect(toListPrefix('media/a/...', 'a/...')).toBe('media/a/...');

	// The empty root stays the whole bucket for a prefix that resolves to nothing
	expect(toListPrefix('', '.')).toBe('');
});

test('toListPrefix built the way a driver builds it never lists a key outside the location', () => {
	// Drivers pass `joinPath(root, confinePath(prefix))` and the raw prefix; every prefix that resolves to the root
...
test('toRelativePath strips the root and its slash, and nothing else', () => {
	// The root and the slash after it go; with no root there is nothing to strip
	...
	// Only `root/` is stripped, not a shared string prefix: a key under `media-archive/` keeps its own root
