...
test('EventContext records the actor and anything the emitter attaches', () => {
	// Only `accountability` is fixed — and may be `null` for an anonymous event; the rest hangs off the index
...
test('FilterHandler may replace the payload or leave it as is', () => {
	// Returning `undefined` keeps the payload untouched, so an inspecting handler need not return anything; the
...
test('ActionHandler and InitHandler may be synchronous or asynchronous', () => {
	// The emitter waits for no action handler and logs a rejection; the init stage awaits the handler, and both
