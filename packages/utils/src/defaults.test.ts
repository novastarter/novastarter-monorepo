...
test('Returns defaults with input properties assigned', () => {
	// A key the caller sets is kept and a key it leaves out is filled in; the result holds both
...
test('Overwrites undefined values in input object', () => {
	// An explicit `undefined`, the shape an unset env variable takes, counts as missing rather than as a value
