...
test('No two entries are the same word in a different casing', () => {
	// 1. The lookup is case-insensitive and answers with the first entry that matches, so a second spelling of the
	//    same word could never be reached
	const lowerCased = specialCase.map((entry) => entry.toLowerCase());
...
test('Holds the mixed-case spellings the acronym list cannot', () => {
	// 1. `FAQs`, `IDs` and `PDFs` are plural acronyms; upper-casing them would give `FAQS`, so they belong here
	expect(specialCase).toEqual(expect.arrayContaining(['FAQs', 'IDs', 'PDFs']));
});
