...
for (const [input, output] of tests) {
	test(`${input} => ${output}`, () => {
		// 1. Each row is one documented example of the readme; the table there and this list must agree
		expect(formatTitle(input)).toBe(output);
	});
}

test('Joins with single spaces whatever the separators around and between the words', () => {
	// 1. Doubled, leading and trailing separators used to leave empty words behind, each joined with a space of its own
	...
	// 2. An empty word used to count as the last word, so the minor word before it stayed lower-cased
	expect(formatTitle('come in ')).toBe('Come In');

	// 3. Nothing but separators, or nothing at all, is an empty title, not a string of spaces
	expect(formatTitle('')).toBe('');
	expect(formatTitle('---')).toBe('');
});

test('Keeps an acronym spelled with a digit whole and splits a capital run before a word, not before a digit', () => {
	// 1. `MP3` used to be broken into `M P3` and `M2M`, a listed acronym, into `M2 M`; `MP3` and `HTML5` are not listed,
...
test('Spells a mixed-case special word as listed', () => {
	// 1. `FAQs` sat in the acronym list, where the upper-cased comparison could never match it
