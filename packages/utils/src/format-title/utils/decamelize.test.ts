...
test('Converts camelcase to underscores', () => {
	// 1. The plain case: one boundary between a lower-case letter and a capital
...
test('Keeps a run of capitals together and splits it from the word that follows', () => {
	// 1. `XML` is one word, `Parser` the next; letter by letter would give `x_m_l_parser`
...
test('Keeps a run of capitals together when digits follow it', () => {
	// 1. A digit is not the start of a word: `MP3` used to be split before its last capital into `m_p3`
...
test('Keeps an acronym spelled with a digit as one word', () => {
	// 1. The capital after the digit starts no lower-case word, so there is no boundary; the acronym list holds these
...
test('Leaves snake_case and plain text alone', () => {
	// 1. Nothing to mark: no capital follows a lower-case letter and there is no run of capitals
