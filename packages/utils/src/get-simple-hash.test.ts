...
test('Returns the same digest for the same input', () => {
	// A cache key built from the digest must be stable across calls, and it is unsigned 32-bit hex: at most eight
...
test('Returns a different digest for a different input', () => {
	// One extra character must change the digest, or two keys would collide on the smallest edit
	expect(getSimpleHash('hello')).not.toBe(getSimpleHash('hello!'));
});

test('Hashes the empty string to zero', () => {
	// No code unit is folded in, so the running value stays at its seed of zero
