...
test('Joins segments with a single forward slash', () => {
	// The plain case, relative and absolute: `/` between segments and nothing else changed
...
test('Collapses repeated and mixed separators', () => {
	// Slashes on both sides of a join, doubled slashes and backslashes all end as one `/`
...
test('Drops a trailing slash', () => {
	// `a/b/` and `a/b` are the same key, so the trailing separator never reaches the result
	expect(joinPath('a', 'b/')).toBe('a/b');
	expect(joinPath('a/b/')).toBe('a/b');
});

test('Skips empty segments', () => {
	// An empty segment adds no separator of its own, or `''` and `'a'` would join as `/a`
	expect(joinPath('', 'a', '', 'b')).toBe('a/b');
	expect(joinPath('a', '')).toBe('a');
});

test('Answers with an empty string for no segments', () => {
	// No segments, or only empty ones, is no path: not the root and not a single slash
	expect(joinPath()).toBe('');
	expect(joinPath('', '')).toBe('');

	// A `.` names the current place, which is no path either
	expect(joinPath('.')).toBe('');
	expect(joinPath('.', '')).toBe('');
});

test('Resolves dot segments', () => {
	// `.` names the current place, so it vanishes from the middle and from the front alike
	expect(joinPath('a', '.', 'b')).toBe('a/b');
	expect(joinPath('./a', './b')).toBe('a/b');
	expect(joinPath('.')).toBe('');
});
...
test('Resolves parent segments against the segments before them', () => {
	// Each `..` pops the segment before it, across argument boundaries, the way `path.posix.join` does
...
test('Keeps a parent segment that climbs above a relative path', () => {
	// Nothing to pop: the `..` stays in front, as `path.posix.join` leaves it
...
test('Drops a parent segment that climbs above the root', () => {
	// There is nothing above `/`, so the climb is ignored and the path stays inside the root
	expect(joinPath('/', '..', 'a')).toBe('/a');
	expect(joinPath('/a', '..', '..')).toBe('/');
});

test('Keeps the root of an absolute path', () => {
	// The leading `/` is not a segment to pop or collapse; it stays whatever follows it
	expect(joinPath('/')).toBe('/');
	expect(joinPath('/', 'a')).toBe('/a');
	expect(joinPath('/a', '..')).toBe('/');
});
...
test('Treats a Windows drive as an ordinary segment', () => {
	// Only a leading `/` is a root: the drive is popped like any segment, which is the documented limit for keys
...
test('Answers with the root for segments that are nothing but separators, like path.posix.join', () => {
	// `normalizePath` collapses `//` to nothing; the root is read from the raw join, so it is not lost with it
	expect(joinPath('/', '/')).toBe('/');
	expect(joinPath('//')).toBe('/');
	expect(joinPath('///', '.')).toBe('/');
	expect(joinPath('\\', '/')).toBe('/');
...
test('confinePath keeps a caller path under the root it is joined to', () => {
	// A leading `..` has nothing to climb once rooted, so it is dropped; the rest resolves as usual
	...
	// Empty and dot-only paths stay empty, so a `list('')` still means the whole root
	expect(confinePath('')).toBe('');
	expect(confinePath('.')).toBe('');
	expect(confinePath('/')).toBe('');

	// Joined under a root, the result never leaves it
...
test('Refuses a segment that is not a string instead of dropping or spelling it out', () => {
	// A missing caller path must not quietly name the root — the storage drivers build their keys with this
