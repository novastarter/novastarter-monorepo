...
export const isReadableStream = (input: unknown): input is Readable => {
	// Only a non-null object can carry stream members; anything else is rejected before its properties are read
	if (input === null || typeof input !== 'object') {
		return false;
	}

	// Probe for members a Node readable always has and plain objects or Web streams lack: the piping API, the
