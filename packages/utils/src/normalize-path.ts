...
export const normalizePath = (
	...
	// A lone separator is the root directory, whichever style it came in — or nothing at all when the leading
	...
	// Keep the Windows `\\?\` / `\\.\` namespace prefix aside: the split below would collapse its double
	...
	// Split on any run of separators, so mixed and doubled slashes collapse into one
	const segments = path.split(/[/\\]+/);

	// A trailing separator leaves an empty last segment; drop it so `a/b/` and `a/b` normalise the same way
	if (segments.at(-1) === '') {
		segments.pop();
	}

	// Nothing but separators — `//`, `/\` — is the root, like the lone separator above, not an empty path
	...
	// Strip the leading slash on request. The check looks at the normalised path, so a backslash-rooted input loses
