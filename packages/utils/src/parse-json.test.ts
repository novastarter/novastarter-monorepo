...
describe('parseJSON', () => {
	it('parses valid JSON object', () => {
		// The plain case takes the fast path: no `__proto__` and no escape in the text, so no reviver runs
		expect(parseJSON('{"key": "value"}')).toEqual({ key: 'value' });
	});

	it('parses valid JSON array', () => {
		// An array is a valid top-level value, not only an object
		expect(parseJSON('[1, 2, 3]')).toEqual([1, 2, 3]);
	});

	it('parses JSON with nested objects', () => {
		// Nesting is left to `JSON.parse`; the wrapper adds nothing that could flatten it
		const input = '{"outer": {"inner": "value"}}';
		expect(parseJSON(input)).toEqual({ outer: { inner: 'value' } });
	});

	it('parses JSON string value', () => {
		// A bare string is JSON too; it must come back unquoted, not wrapped or rejected
		expect(parseJSON('"hello"')).toBe('hello');
	});

	it('parses JSON number value', () => {
		// A bare number is answered with as a number, not as its text
		expect(parseJSON('42')).toBe(42);
	});

	it('parses JSON boolean values', () => {
		// Both literals come back as booleans, so `false` is not confused with a missing value
		expect(parseJSON('true')).toBe(true);
		expect(parseJSON('false')).toBe(false);
	});

	it('parses JSON null value', () => {
		// `null` is a value of its own, distinct from `undefined`, and must survive the parse as such
		expect(parseJSON('null')).toBe(null);
	});

	it('throws on invalid JSON', () => {
		// The `SyntaxError` of `JSON.parse` is passed on; the caller decides whether to fall back
		expect(() => parseJSON('invalid')).toThrow();
	});

	it('throws on empty string', () => {
		// Empty text is not JSON either; answering `undefined` would hide a missing value
		expect(() => parseJSON('')).toThrow();
	});

	it('strips __proto__ property from parsed JSON', () => {
		// The text carries the key, so the reviver path runs and drops it while keeping every other key
		const malicious = '{"__proto__": {"polluted": true}, "safe": "value"}';
		const result = parseJSON(malicious) as { safe: string };
		expect(result.safe).toBe('value');

		// An own-property check rather than `in` or `result.__proto__`: those would see `Object.prototype` and
		// answer `true` for any object
		expect(Object.prototype.hasOwnProperty.call(result, '__proto__')).toBe(false);

		// Had the key gone through, `Object.prototype` itself would now carry `polluted` for every object
		expect(({} as { polluted?: unknown }).polluted).toBeUndefined();
	});

	it('strips nested __proto__ properties', () => {
		// The reviver sees every key at every depth, so a key hidden one level down is dropped as well
	...
	it('handles JSON without __proto__ using fast path', () => {
		// Harmless text with nesting takes the fast path and must parse the same as with the reviver
	...
	it('handles __proto__ as part of another key name', () => {
		// The substring switches the reviver on, but only the exact key `__proto__` is dropped, not one containing it
...
describe('parseJSON with escaped keys', () => {
	it('drops a __proto__ key spelled with unicode escapes as well', () => {
		// `JSON.parse` resolves `_` to `_` before the reviver sees the key; a substring check on the raw text
...
describe('noproto', () => {
	it('returns value for non-__proto__ keys', () => {
		// An ordinary key passes its value through, so the reviver changes nothing in a harmless document
		expect(noproto('key', 'value')).toBe('value');
	});

	it('returns undefined for __proto__ key', () => {
		// `undefined` from a reviver deletes the key; that is how the prototype key is dropped
		expect(noproto('__proto__', 'value')).toBeUndefined();
	});

	it('returns value for keys containing __proto__ as substring', () => {
		// Only the exact key is dangerous; a key merely containing the text is a legitimate property
		expect(noproto('my__proto__key', 'value')).toBe('value');
	});

	it('handles various value types', () => {
		// The value is returned as it is, whatever its type: the reviver decides by key alone
