...
export interface RetryOptions {
	/**
	 * Attempts allowed after the first one; `0` runs the operation once.
	 *
	 * @defaultValue 3
	 */
	retries?: number;
 ...
 *
 * @defaultValue 3 retries, 100 ms base pause doubling each time, no ceiling, no jitter, every error retried.
 */
 ...
 * `options.jitter` is outside `0` to `1`.
 * @example
 * ```ts
...
export const retry = async <T>(fn: (attempt: number) => Promise<T>, options: RetryOptions = {}): Promise<T> => {
	...
	// 2. A jitter above one would let a pause come out negative, which `setTimeout` silently rounds up to zero; a
	...
	// 3. A signal aborted before the call: fail right away rather than running an attempt nobody waits for
	if (signal?.aborted) {
		throw signal.reason;
	}

	// 4. Attempts are numbered from one, so the first retry is attempt two and the last allowed is `retries + 1`
	for (let attempt = 1; ; attempt++) {
		try {
			return await fn(attempt);
		} catch (error) {
			// 5. Out of budget, or an error not worth retrying: hand it over untouched
			if (attempt > retries || !shouldRetry(error, attempt)) {
				throw error;
			}

			// 6. The base pause: a function decides itself from the number of the failed attempt, a number grows by
			//    `factor` per retry
			const base = typeof delay === 'function' ? delay(attempt) : delay * factor ** (attempt - 1);

			// 7. Spread it by `jitter` — a factor drawn evenly between `1 - jitter` and `1 + jitter`, never below zero
			//    since `jitter` is at most one, so the mean pause stays what the caller asked for — and cap the result,
			//    so the cap bounds the spread as well as the growth
			const spread = jitter === 0 ? base : base * (1 + (Math.random() * 2 - 1) * jitter);
			const pause = Math.min(spread, maxDelay);

			// 8. Report, then wait: the callback sees the pause that is about to happen
