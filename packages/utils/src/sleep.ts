...
export const sleep = (ms: number, signal?: AbortSignal): Promise<void> => {
	return new Promise((resolve, reject) => {
		// A wait the timer cannot hold is refused rather than turned into no wait: Node arms 1 ms for a negative,
		...
		// A signal aborted before the call: fail right away rather than arming a timer nobody waits for
		if (signal?.aborted) {
			reject(signal.reason);
			return;
		}

		// On fire, the abort listener is dropped so a later abort of a long-lived signal finds nothing to call.
		const timer = setTimeout(() => {
			// The wait is over: the listener goes first, so an abort arriving now finds nothing to reject
			signal?.removeEventListener('abort', onAbort);
			resolve();
		}, ms);

		// The rejection carries the signal's reason, so the caller sees why the wait ended.
		function onAbort(): void {
			// The timer goes first, so it cannot resolve a promise this rejection is about to settle
			clearTimeout(timer);
			reject(signal?.reason);
		}
