...
test('Answers with the message of an Error', () => {
	// 1. Subclasses count as errors too; only the message is taken, never the class name or the stack
...
test('Falls back to the name of an Error without a message', () => {
	// 1. An empty message would leave a wrapped message or a log line ending in nothing; the class name says at
...
test('Writes non-Error values as text', () => {
	// 1. What vendor SDKs throw instead of errors: strings, numbers, plain objects
...
test('Falls back to the object tag for a value String() cannot convert', () => {
	// 1. An object without a prototype has no `toString`, so `String()` throws; describing an error must not
	expect(toErrorMessage(Object.create(null))).toBe('[object Object]');
});

test('Names an absent value instead of answering with nothing', () => {
	// 1. `undefined` and `null` in a log line say more than an empty message
