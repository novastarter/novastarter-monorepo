...
export const toErrorMessage = (error: unknown): string => {
	try {
		// An `Error` says what went wrong in its `message`; the stack is not part of the answer, and the class name
		// only stands in when the message is empty, so the line never ends in nothing
		if (error instanceof Error) {
			return error.message || error.name;
		}

		// Anything else is written as text with `String()`, so a thrown `undefined` reads as `'undefined'` — the
		// word says more in a log line than an empty message would
		return String(error);
	} catch {
		// `String()` calls `toString()`, which an object without a prototype does not have, and a revoked `Proxy`
