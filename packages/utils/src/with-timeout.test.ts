...
const resolveAfter = <T>(ms: number, value: T): Promise<T> => {
	return new Promise((resolve) => {
		// A plain timer, driven by the fake clock like the deadline it races
		setTimeout(() => resolve(value), ms);
	});
};

test('Resolves with the value when the operation finishes before the deadline', async () => {
	// The work takes 100 ms of a 1000 ms budget; the value comes through and the deadline timer is gone
...
test('Rejects with what the operation rejected with', async () => {
	// The operation's own error comes through untouched, and the deadline timer is cleared with it
...
test('Rejects with a TimeoutError once the deadline passes, not before', async () => {
	// The work never finishes; the promise stays pending short of the deadline and fails exactly at it
	const settled = vi.fn();
...
test('Rejects with what the error factory makes, given the deadline', async () => {
	// The factory sees the limit and its result is what the caller gets, so a package throws its own error class
...
test('Rejects with what the error factory throws instead of crashing the timer', async () => {
	// A factory that throws is a bug in the caller; it surfaces as the rejection, not as an uncaught exception
...
test('Hands a function operation a signal that aborts with the timeout error at the deadline', async () => {
	// The function starts the work with the signal; on timeout the signal's reason is the same error the caller
...
test('Rejects with the abort reason when the outer signal aborts during the wait, aborting the inner signal too', async () => {
	// Aborting mid-wait ends it at once with the signal's reason, passes the reason to the operation's signal and
...
test('Rejects at once with a signal already aborted, without starting the operation', async () => {
	// No timer is armed and a function operation is never called when there is nothing to wait for
	const controller = new AbortController();
	...
	// A promise handed in with an aborted signal is still observed: were its rejection left unhandled, Vitest
...
test('Rejects with what a function operation throws before returning a promise', async () => {
	// A synchronous throw fails the call right away, with the deadline timer already cleared
	const wait = withTimeout(() => {
...
test('Resolves with a plain value a function operation answers with, clearing the timer', async () => {
	// The type demands a promise, but a callback that returns a value must not break the settle or leak the timer
...
test('Removes its abort listener once the operation settled, so a shared signal keeps no reference', async () => {
	// The listener is there while the wait runs and gone on settlement, so a late abort finds nothing to reject
...
test('Refuses a deadline the timer cannot hold instead of firing it at once', async () => {
	// Negative, NaN and overlong deadlines would all become a 1 ms timer in Node; each is a RangeError, and a
	...
	// A function operation is never started for a deadline that was refused
