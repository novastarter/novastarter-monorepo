...
describe('field and path', () => {
	test('first path segment becomes the field, the rest the path', () => {
		// Joi's path starts at the payload root, so the first key is the field and the rest points into it
	...
	test('prepends the given parent path', () => {
		// A caller validating a nested object separately passes where it sits, so the path stays absolute
...
describe('any.only', () => {
	test('maps a single value to eq', () => {
		// One allowed value is an equality, reported with the scalar
	...
	test('maps several values to in', () => {
		// A list of allowed values is `_in`, reported with the whole list
	...
	test('maps an empty allow list to in with no values', () => {
		// The never-validating fallback of a malformed rule allows nothing; that is `in` over an empty list, not an
	...
	test('maps a number and its string twin to eq with the first entry', () => {
		// `generateJoi` builds `_eq: 18` as `[18, '18']`; that is still one value, in the caller's form
		...
		// Only numbers get a twin, so `true` next to `'true'` is still a list of two
	...
	test('maps a boolean to eq with its text, like the zod converter reports it', () => {
		// `_eq: true` is built as the boolean itself; the extensions shape holds numbers and strings only, so the
	...
	test('maps booleans in a list to in with their texts', () => {
		// A genuine list keeps its size, and each entry is stringified the same way as the scalar case
	...
	test('maps null to null and empty string to empty', () => {
		// The `_null` / `_empty` operators are allow lists with a single entry, told apart by the entry itself
...
describe('any.invalid', () => {
	test('maps a single value to neq', () => {
		// One forbidden value is an inequality, reported with the scalar
	...
	test('maps several values to nin', () => {
		// A list of forbidden values is `_nin`, reported with the whole list
	...
	test('maps a number and its string twin to neq with the first entry', () => {
		// `generateJoi` builds `_neq: 18` as `[18, '18']`; that is still one forbidden value
	...
	test('maps a boolean to neq with its text, like the zod converter reports it', () => {
		// `_neq: false` is built as the boolean itself and stringified for the extensions, mirroring the zod side
	...
	test('maps null to nnull and empty string to nempty', () => {
		// The `_nnull` / `_nempty` operators are deny lists with a single entry, told apart by the entry itself
...
describe('ranges', () => {
	test.each([
		...
		// The rule is matched on its suffix, so number and date bounds share one mapping and the bound is `limit`
	...
	test('normalises a date bound to the ISO string the caller passed', () => {
		// Joi puts the Date object itself into `context.limit` for date bounds; the extensions hold numbers and
...
describe('substrings', () => {
	test('maps contains, icontains and ncontains with the substring', () => {
		// The extended Joi puts the substring into the context, so it is passed through as is
		expect(joiValidationErrorItemToErrorExtensions(item('string.contains', { substring: 'x' }))).toMatchObject({
			type: 'contains',
			substring: 'x',
		});

		// `icontains` ends with `contains`; the whole rule name is compared, so it keeps its own operator
		expect(joiValidationErrorItemToErrorExtensions(item('string.icontains', { substring: 'x' }))).toMatchObject({
			type: 'icontains',
			substring: 'x',
		});

		// `ncontains` ends with `contains` too and must not be swallowed by the contains branch
	...
	test.each([
		...
		// The rule name is the operator; the substring comes from the context untouched, so metacharacters and
	...
	test('does not mistake a named pattern for a prefix rule', () => {
		// Hand-built named patterns are no longer reverse-engineered; they are unknown like any other rule
		expect(() =>
			joiValidationErrorItemToErrorExtensions(item('string.pattern.name', { name: 'starts_with', regex: /^a.*/ })),
		).toThrowError(
			/^\[@novastarter\/validation\] joiValidationErrorItemToErrorExtensions: Joi rule "[^"]+" has no matching validation error type$/,
		);
	});
});
...
describe('other rules', () => {
	test('maps a bare pattern to regex with the pattern, like the zod converter reports it', () => {
		// The pattern is reported, so the client can show what the value had to match — the same `invalid` the zod
	...
	test('maps a wrong base type to required', () => {
		// A value of the wrong type is as unusable as a missing one, so both read as `required`
	...
	test('maps a value matching no alternative type to required', () => {
		// The substring rules are string-or-array alternatives; failing both by type is a wrong base type
		expect(
	...
	test('maps the _nbetween alternatives pair to nbetween with the two bounds', () => {
		// A value inside the range fails both alternatives; Joi wraps the pair as `alternatives.match` and the
	...
	test('normalises the date bounds of the _nbetween alternatives pair to ISO strings', () => {
		// Date bounds arrive as `Date` objects, exactly like the single-bound date rules, and normalise the same way
	...
	test('still throws on an alternatives pair it cannot describe', () => {
		// An `alternatives.match` whose wrapped details are no `less` / `greater` pair is not `_nbetween`; it must
	...
	test('maps an empty string rejected by a stock string schema to nempty', () => {
		// A caller's own `Joi.string()` rejects `''` before any rule; that is "must not be empty" to the client and
	...
	test('maps the array forms of contains and ncontains', () => {
		// Joi reports the array rules without the substring, so only the operator can be given back
		expect(
		...
		// The forbidden item is reported at its index, which lands in the path
	...
	test('maps number.unsafe and number.infinity to unsafe', () => {
		// Neither an out-of-range nor an infinite number is one the client can act on; both read as `unsafe`
	...
	test('throws on a rule it cannot describe', () => {
		// An unknown rule must fail loudly rather than produce an error without a type
